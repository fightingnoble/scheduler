...
def main():
    ...
    from task.task_agent import TaskIntAttr
    # "interleave" if task_attr.name != "Steering_speed" else "repeat"
    def div_mod_fn(task_attr:TaskIntAttr, f_gcd:int):
        if task_attr.name != "Steering_speed":
            return "interleave"
        else:
            return "repeat"
    glb_n_task_dict = gen_taskint_from_cfg(taskattr_dict, f_gcd, div_mod_fn=div_mod_fn)
    # ***************** artifiact: frame isolation *******************************
