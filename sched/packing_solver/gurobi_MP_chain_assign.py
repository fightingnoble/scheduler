...
class GurobiRscSlackEstim():
    ...
    def __init__(self, K, flops, e2e, margin, constr_core:Dict[str, Union[str, int, List[int]]], tot_cores):
        # constants
        self.K = K
        self.flops = flops
        self.e2e = e2e
    ...
    def define_constraints(self):
        ...
        self.model.addConstr(self.e2e >= sum([self.lat[i] / (1 - self.margin[i]) for i in range(self.K)]), name="e2e")
    ...
    def solve(self):
        ...
        try:
            ...
            sol = {}
            for i in range(self.K):
                sol[i] = (self.core[i].x, self.lat[i].x)
            return sol
...
if __name__ == "__main__":
    ...
    for chain in chains:
        ...
        chains_info2.append((chain, copy.deepcopy(flops_dict), slcak_rem, ops_rem, is_ddl_constr))
    ...
    for (chain, flops_dict, slcak_rem, ops_rem, is_ddl_constr), (chain2, flops_dict2, slcak_rem2, ops_rem2, is_ddl_constr2) in zip(chains_info, chains_info2):
        for node in chain:
        ...
        margin = [temporal_rda_ratio for _ in range(K)]
        ...
        solver = GurobiRscSlackEstim(K, flops, slcak_rem2, margin, constr_core, tot_cores)
