...
def extract_min_cores(args, root_dir, method='glb_dyn', jitter_en='en'):
...
glb_min_cores, glb_ctx_switch_stat = extract_min_cores(args, root_dir=args.root_dir_glb, method='glb_dyn')
dyn_min_cores, dyn_ctx_switch_stat = extract_min_cores(args, root_dir=args.root_dir_dyn, method='dyn')
glb_min_cores_static, glb_ctx_switch_stat_static = extract_min_cores(args, root_dir=args.root_dir_glb, method='glb_dyn', jitter_en='dis')
dyn_min_cores_static, dyn_ctx_switch_stat_static = extract_min_cores(args, root_dir=args.root_dir_dyn, method='dyn', jitter_en='dis')

# merge the dataframes
min_cores = pd.concat([glb_min_cores, glb_min_cores_static, dyn_min_cores, dyn_min_cores_static], axis=1)
min_cores.columns = pd.MultiIndex(levels=[['glb', 'glb_static', 'dyn', 'dyn_static'], ['min_cores']], codes=[[0, 1, 2, 3], [0, 0, 0, 0]], names=['method', 'stat'])
...
ctx_switch_stat.columns = pd.MultiIndex.from_product([['glb', 'glb_static', 'dyn', 'dyn_static'], ['avg_cum_time', 'min_cum_time', 'max_cum_time']], names=['method', 'stat'])
# set column names
