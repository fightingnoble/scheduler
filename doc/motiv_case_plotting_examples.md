...
### 功能
绘制 **并排双柱状图**，对比不同预留分位数下的idle_ratio和miss_ratio，清晰展示静态方法在利用率和可靠性之间的权衡关系。
...
### 图表特点
- **并排双柱状图**：每个预留分位数对应两个柱子
  - 橙色柱（左）：Idle Ratio（闲置算力占比）
  - 红色柱（右）：Miss Ratio（miss负载占比）
- **数值标注**：每个柱子顶部显示精确数值
- **参考线**：红色虚线标记10%阈值
- **期望结果**：
  - Idle柱随预留分位数增加而升高（p50→p99）
  - Miss柱随预留分位数增加而降低
  - 展示清晰的权衡关系：高预留→高idle+低miss
